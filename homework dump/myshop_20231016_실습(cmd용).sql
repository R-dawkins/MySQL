...
select customer_name,customer_id,gender,city,phone,point from customer where not city = '서울' and point >= 500000;
-- Q16) 포인트가 400,000 이상인 '서울' 지역 남자 고객의 이름, 아이디, 성별, 지역, 전화번호, 포인트를 조회하세요.
select customer_name,customer_id,gender,city,phone,point from customer where city = '서울' and point >= 400000 and gender = 'M';
...
select * from customer where left(birth_date,4) = '1990' and city in('대구','경주') and gender = 'M';
...
select count(*), sum(point) from customer where city = '서울';
-- Q05) '서울' 지역 고객의 포인트 합, 평균, 최댓값, 최솟값을 조회하세요.
select count(*), sum(point), avg(point), max(point), min(point) from customer where city = '서울';
...
select sum(point) from customer group by gender;
...
select left(order_date,4) year, substring(order_date,6,2) mm,sum(total_due) from order_header where left(order_date,7) between '2019-01' and '2019-06' group by year,mm;
-- Q19) 2019.01 ~ 2019.06 기간 주문에 대하여 주문연도별, 주문월별 전체금액 합과 평균을 조회하세요.
select left(order_date,4) year, substring(order_date,6,2) mm,sum(total_due),avg(total_due) from order_header where left(order_date,7) between '2019-01' and '2019-06' group by year,mm;
-- Q20) 주문연도별, 주문월별 전체금액 합과 평균을 조회하고, rollup 함수를 이용하여 소계와 총계를 출력해주세요.
select left(order_date,4) year, substring(order_date,6,2) mm,sum(total_due),avg(total_due) from order_header group by year,mm with rollup;

/**
	테이블 조인
*/
-- Q01) 전체금액이 8,500,000 이상인 주문의 주문번호, 고객아이디, 사원번호, 주문일시, 전체금액을 조회하세요.
select o.order_id,c.customer_id,e.employee_id,o.order_date,o.total_due from order_header o inner join customer c inner join employee e
on o.customer_id = c.customer_id and e.employee_id = o.employee_id and o.total_due >= 8500000;
-- Q02) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 고객이름도 같이 조회되게 수정하세요.
select o.order_id,c.customer_id,c.customer_name,e.employee_id,o.order_date,o.total_due from order_header o inner join customer c inner join employee e
on o.customer_id = c.customer_id and e.employee_id = o.employee_id and o.total_due >= 8500000;
-- Q03) Q01 쿼리를 복사해 붙여 넣은 후 직원이름도 같이 조회되게 수정하세요.
select o.order_id,c.customer_id,e.employee_name,e.employee_id,o.order_date,o.total_due from order_header o inner join customer c inner join employee e
on o.customer_id = c.customer_id and e.employee_id = o.employee_id and o.total_due >= 8500000;
-- Q04) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 고객이름, 직원이름도 같이 조회되게 수정하세요.
select o.order_id,c.customer_id,c.customer_name,e.employee_name,e.employee_id,o.order_date,o.total_due from order_header o inner join customer c inner join employee e
on o.customer_id = c.customer_id and e.employee_id = o.employee_id and o.total_due >= 8500000;
-- Q05) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 전체금액이 8,500,000 이상인 '서울' 지역 고객만 조회되게 수정하세요.
select o.order_id,c.city,c.customer_id,c.customer_name,e.employee_name,e.employee_id,o.order_date,o.total_due from order_header o inner join customer c inner join employee e
on o.customer_id = c.customer_id and e.employee_id = o.employee_id and o.total_due >= 8500000 and city ='서울';
...
select p.product_id,p.product_name,p.sub_category_id,s.sub_category_name,c.category_name from product p inner join sub_category s inner join category c;
-- Q11) 다정한 사원이 2019년에 주문한 상품명을 모두 출력해주세요.
select e.employee_id,e.employee_name,h.order_date,p.product_name,h.order_id,p.product_id from employee e inner join order_header h inner join order_detail d inner join product p
on e.employee_id = h.employee_id and h.order_id = d.order_id and p.product_id = d.product_id and employee_name ='다정한' and order_date like '2019%';
-- Q12) 청소기를 구입한 고객아이디, 사원번호, 주문번호, 주문일시를 조회하세요.
select c.customer_id,e.employee_id,h.order_id,h.order_date,p.product_name from customer c inner join order_header h inner join employee e inner join order_detail d inner join product p
on c.customer_id = h.customer_id and e.employee_id = h.employee_id and h.order_id = d.order_id and p.product_id = d.product_id and p.product_name like '__청소기';
