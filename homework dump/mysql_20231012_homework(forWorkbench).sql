...
select emp_id, gender, phone, email, salary, emp_name from employee where emp_name = '홍길동';
...
select emp_name, hire_date, ifnull(salary, 0) from employee where salary >= 5000 and hire_date >= '2015-01-01' order by hire_date, salary desc;
-- 입사일이 2016년 1월 1일 이후 이고, 연봉이 6000 이상인 사원들을 조회
-- 최근 입사일 기준으로 정렬
select * from employee where hire_date >= 20160101 and salary >= 6000 order by hire_date;
-- 입사일이 2016년 1월 1일 이후 이거나, 연봉이 6000 이상인 사원들을 조회
-- 최근 입사일 기준으로 정렬
select * from employee where hire_date >= 20160101 or salary >= 6000 order by hire_date;
...
-- 보너스가 1000 이상인 사원 조회
select emp_id, emp_name, hire_date, email, ifnull(salary, 50), salary*0.2 from employee;
select emp_id, emp_name, hire_date, email, salary, salary*0.2 from employee where salary*0.2 >= 1000;
...
select * from employee where phone like '010%' and dept_id = 'mkt';
...
select format(rand()*100000,0);
-- 사원아이디가 짝수인 사원들만 조회 : 사원아이디 S0001의 숫자만 추출하여 나머지 함수를 적용
select emp_id, emp_name from employee where mod(right(emp_id,4),2) = 0;
...
-- 사원이름과 사원아이디를 결합하여 [홍길동(S0001)], 연봉 컬럼을 [8,500만원(보너스10%)] 형식으로 출력 출력
 select concat('[',emp_name,'(',emp_id,')',']'), concat('[',salary,'만원','(',salary*0.1,'%',')',']') from employee;
-- 문자열 추출 : substring(문자열, 위치, 문자열수)

-- 사원아이디, 사원명, 입사년도, 폰번호, 이메일 조회
 select emp_id, emp_name, substring(hire_date,1,4), phone, email from employee;
 -- 2015년도 입사자들 모두 조회
...
select trim(leading '&' from '&&&&&&&&&&&&&&&&&&&&&&&&&대 한 민 국           ');
-- '대 한 민 국$$$$$$$$$$$$$$$$$$$$$$$$' 문자열의 앞에 있는 $ 기호 삭제
 select trim(trailing '$' from '대 한 민 국$$$$$$$$$$$$$$$$$$$$$$$$');
